...
function TimelineItemComponent({ item, index }: TimelineItemProps) {
  ...
  return (
    ...
    >
      {/* Timeline dot */}
      <div className="flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 bg-slate-300 border-2 border-slate-700 rounded-full flex items-center justify-center relative z-10">
        <Image
        ...
        <CardContent className=" p-4 md:p-6 md:w-auto ">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 space-y-2 sm:space-y-0">
            ...
            <div className="flex flex-col sm:items-end space-y-2">
              <Badge
                variant="outline"
                className={`${typeColors[item.type]} text-xs sm:text-sm w-fit`}
              >
                {item.date}
              </Badge>
            </div>
          ...
          <p className="text-slate-300 mb-3 text-sm sm:text-base leading-relaxed">
            {item.description}
          </p>
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {Array.from({ length: 3 }).map(
        (
          _,
          index // Adjust length based on how many loading skeletons you want to show
        ) => (
          <div
            key={index}
            className="relative flex items-start space-x-4 sm:space-x-6 animate-pulse"
          >
    ...
    return (
      <Section id="parcours" containerSize="lg">
        ...
        <div className="text-red-500 text-center">
          Une erreur s'est produite lors du chargement de votre parcours.
